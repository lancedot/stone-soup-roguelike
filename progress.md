...
- Multi-edge wall pass: wall cells now render every open-side half wall instead of choosing only the first orientation, fixing missing half-wall pieces at concave corners and T-junctions. Verified with 25/25 tests, Vite build, single-file export, and screenshot `output/multi-edge-wall-smoke/shot-0.png`.
- Full-size corner pass: restored wall corner caps from 32px half-grid crops to full 64px L-shaped corner pieces so outer corners connect to adjacent half walls. Verified with 25/25 tests, Vite build, single-file export, and screenshot `output/full-corner-wall-smoke/shot-0.png`.
- Wall autotile rebuild: generated 15 mask-based wall tiles with 4 variants each from north/south/side pieces so each wall cell renders one precomposed Zelda/Isaac-style connector tile instead of runtime-stacked half-wall images. Verified with 25/25 tests, Vite build, single-file export, autotile preview `output/wall-autotile-preview.png`, and gameplay screenshot `output/wall-autotile-smoke/shot-0.png`.
- Wall seam alignment: locked wall autotile rendering to a single canonical variant so adjacent wall cells share the same brick seam baseline instead of jumping between variants. Verified with 25/25 tests, Vite build, single-file export, and screenshot `output/wall-seam-lock-smoke/shot-0.png`.
